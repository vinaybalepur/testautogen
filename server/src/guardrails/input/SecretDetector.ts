...
export class SecretDetector implements IInputGuardrail {
  ...
  private readonly patterns: SecretPattern[] = [
    ...
    { name: 'AWS Secret Key',    regex: /(?i)aws(.{0,20})?(?-i)['\"][0-9a-zA-Z\/+]{40}['\"]/.source,     mask: '[AWS_SECRET REDACTED]'    },
    ...
    { name: 'DB Password',       regex: /(?i)(db_password|database_password|db_pass)\s*[=:]\s*\S+/g,      mask: '[DB_PASS REDACTED]'       },
